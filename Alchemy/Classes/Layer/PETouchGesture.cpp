...
bool PETouchGesture::gestureDetector(Touch* pTouch, int event)
{
    bool ret = false;
    static int gesture_state = 0;
    static Vec2 pos[2];
    static _STRUCT_TIMEVAL event_time[2];
    switch (gesture_state) {
        case 0:
			if(event == PE_TOUCH_UP)
            {
				pos[0] = pTouch->getLocation();
				gettimeofday(&event_time[TOUCH_GESTURE_START], NULL);
				if(event == PE_TOUCH_UP)
					gesture_state = 1;
			}
			break;
		case 1:
			gettimeofday(&event_time[TOUCH_GESTURE_END], NULL);
			switch (event) {
				case PE_TOUCH_DOWN:
				{
					pos[1] = pTouch->getLocation();
					double diffTime = time_interval(event_time[TOUCH_GESTURE_START], event_time[TOUCH_GESTURE_END] );
